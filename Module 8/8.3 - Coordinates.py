...
def get_airport_coordinates(icao_code):
    sql=f"SELECT ident,name,latitude_deg,longitude_deg FROM airport WHERE ident={icao_code} "
...
def get_airport_coordinates(icao_code1):
    sql = f"SELECT ident,name,latitude_deg,longitude_deg FROM airport WHERE ident={icao_code1} "
    cursor = connection.cursor()
    cursor.execute(sql)
    result = cursor.fetchall()
    if result:
